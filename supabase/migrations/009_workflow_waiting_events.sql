-- M8.2: Workflow waiting events

create table public.workflow_waiting_events (
...
create index workflow_waiting_events_project_id_idx
  on public.workflow_waiting_events (project_id);

create index workflow_waiting_events_workflow_instance_id_idx
  on public.workflow_waiting_events (workflow_instance_id);

create index workflow_waiting_events_status_idx
  on public.workflow_waiting_events (workflow_instance_id, status)
  where deleted_at is null;

create unique index workflow_waiting_events_active_instance_uidx
  on public.workflow_waiting_events (workflow_instance_id)
  where deleted_at is null
    and status = 'ACTIVE';
