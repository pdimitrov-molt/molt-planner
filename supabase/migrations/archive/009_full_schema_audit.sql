-- 009: Full application schema audit — adds every column the app reads/writes
-- that may be missing from databases stopped at migration 005 or earlier.
--
-- AUDIT REPORT (app expected vs base schema through 005):
--
-- Clients (+4 M1 columns):
--   + contact_viber
--   + contact_whatsapp
--   + secondary_contact
--   + client_insights
--
-- Projects (+7 M1 columns, +6 domain columns if 002/005 never applied):
--   + project_number
--   + category
--   + object_type
--   + package
...
--   + execution_deadline
--   + move_in_date
--   + client_id          (if missing)
--   + engagement_status  (if missing)
--   + priority           (if missing)
--   + site_address       (if missing)
--   + site_area          (if missing)
--   + target_handover_date (if missing)
--
-- Rooms: (none — schema matches app)
--
-- Phases (+1 if migration 004 never applied):
--   + estimated_hours
--
-- Tasks: (none — schema matches app)
--
-- Rules: ALTER TABLE only, ADD COLUMN IF NOT EXISTS, no drops, no data loss.
...
-- M1 taxonomy backfills from legacy project_type
update public.projects
set category = case
...
-- =============================================================================
-- PHASES (rooms and tasks require no additive columns)
-- =============================================================================
