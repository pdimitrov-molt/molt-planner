...
import { updateProjectDeadlinesSettingsAction } from "@/features/project-settings/actions/project-settings.actions";
...
interface SettingsDeadlinesSectionProps {
  settings: ProjectSettingsView;
}

export function SettingsDeadlinesSection({ settings }: SettingsDeadlinesSectionProps) {
  ...
  function save(next: typeof form) {
    startTransition(async () => {
      const result = await updateProjectDeadlinesSettingsAction({
  ...
  return (
    <SettingsSection id="settings-deadlines" title={bg.projects.settings.sections.deadlines}>
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          ...
          <div className="grid gap-2">
            ...
            <Input
              ...
              disabled={isPending}
              onChange={(event) => update("move_in_date", event.target.value || null)}
            />
          </div>
        </div>
